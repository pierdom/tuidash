...
- Layout: host cards (`#homelab-top`, 60% of page) → `FleetStatusWidget` strip (`#homelab-strip`, auto height) → Tailscale + Hetzner (`#homelab-bottom`, 1fr). Inside `#homelab-top`, scarif/bespin/endor stack vertically (desktop) — see `HomelabHostWidget`'s scroll-mode override below for how the split between them actually works.
...
- Desktop height split (added 2026-09-20, after `#homelab-scarif { height: 65% }` kept leaving a visible blank gap when scarif's content was shorter than its fixed share): `HomelabHostWidget` overrides `_sync_scroll_mode` so the **central** card's inner `#host-scroll` gets `height: auto; max-height: 24` instead of the base class's `height: 1fr` — it now sizes to its own content (still internally scrollable past 24 rows), while `#homelab-scarif { height: auto }` in the screen CSS lets the outer widget shrink to match. Bespin/endor keep the base class's `1fr` unchanged, so they grow into whatever scarif doesn't use rather than being stuck at a fixed ratio. Mobile is untouched — the override checks `self.screen.has_class("mobile")` and falls through to `super()._sync_scroll_mode()`, so mobile's flat `height: 9` cap (from `app.py`'s `.mobile #homelab-top HomelabHostWidget` rule, higher specificity than the screen's own `#homelab-scarif` id) still applies uniformly to all three cards.
